...
# Vercel için veritabanı URI ayarı (PostgreSQL veya SQLite fallback)
...
def sync_champions_league(requested_season=None):
    ...
    response = football_data_get(f"competitions/{CHAMPIONS_LEAGUE_CODE}/matches", params={"dateFrom": today, "dateTo": future_date})
    matches = response.get("matches", [])

    if not matches:
        season_to_use = requested_season or api_season
        if season_to_use:
            response = football_data_get(f"competitions/{CHAMPIONS_LEAGUE_CODE}/matches", params={"season": season_to_use})
            matches = response.get("matches", [])
...
def admin():
    return render_template(
        ...
        upcoming_matches=Mac.query.filter(Mac.baslangic.isnot(None), Mac.baslangic > utc_now_naive(), Mac.lig_id == CHAMPIONS_LEAGUE_ID, ~Mac.durum.in_(["bitti", "iptal", "ertelendi"])).count()
    )
